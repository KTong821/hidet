...
class RegistryEntry:
    ...
    For example, model_category could be "image_classification", under which "resnet" is a model_name. The "resnet" module
    could contain class ResNetImageProcessor representing a callable for processing images.
...
class Registry:
    ...
    def load_module(cls, config: PretrainedConfig, module_type: ModuleType = ModuleType.MODEL):
        ...
        model_category, module_name, klass = astuple(
            cls.module_registry[module_type][architecture]
        )

        module = importlib.import_module(
            f"hidet.apps.{model_category}.{module_type.value}.{module_name}"
        )

        if klass not in dir(module):
            raise KeyError(
                f"No processor class named {klass} found in module {module}."
            )
