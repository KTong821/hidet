...
class ResNetEmbeddings(nn.Module[Tensor]):
    ...
    def __init__(self, config: ResNetConfig):
        ...
        self.embedder = ResNetConvLayer(
            config.num_channels, config.embedding_size, kernel_size=7, stride=2
        )
    ...
    def __init__(
        ...
        layer = [
            ...
            ResNetConvLayer(reduces_channels, reduces_channels, stride=stride),
                ResNetConvLayer(
                    reduces_channels, out_channels, kernel_size=1, activation=False
                )
        ]
...
class ResNetStage(nn.Module[Tensor]):
    ...
    def __init__(
        ...
        if config.hidden_act != "relu":
            raise NotImplementedError(
                "Only ReLU supported for ResNet activation."
            )
